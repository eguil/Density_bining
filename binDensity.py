...
\   - var        - variable
    ...
    drift_data = npy.tile(drift_data, (drift_data.shape, 12))
    #mean_data = npy.repeat(mean_data,shape_data,12)
    mean_data = npy.tile(mean_data, (mean_data.shape, 12))
...
def densityBin(fileT,fileS,fileFx,targetGrid='none',fileV='none',outFile='out.nc',debug=True,timeint='all',mthout=False,gridfT='none',gridfS='none',gridfV='none', varNames=['thetao','so'],dedrift=[' '],TctoTp=False, SatoSp=False):
    ...
    - varsNames                 - names of temp and salinity variables (['thetao','so'])
    - dedrift <optional>        - array [swdedrift, fileT, fileS, meanT, meanS, branch_times,member]
        - swdedrift                  - 'on' or 'off'
        - fileT, fileS, meanT, meanS - dedrift files and mean state files
        - branch_times               - parent branch time for drift
        - member                     - member number
        - startdepth_idx             - z index to start defrit from
    - TctoTp <optional>         - boolean, transform conservative temperature into potential temperature and Sa into Sp using Teos-10 gsw library
    ...
    if gridfT != 'none':
        ft2 = cdm.open(gridfT)
        fs2 = cdm.open(gridfS)
        thetao_h    = ft2[varNames[0]] # Create variable handle
        so_h        = fs2[varNames[1]]
    else:
        thetao_h    = ft(varNames[0], time = slice(1,10)) ; # remove handle for non cmor files
        so_h        = fs(varNames[1], time = slice(1,10)) ; #
    if fileV != 'none':
        if gridfV != 'none':
            fv2 = cdm.open(gridfV)
            vo_h        = fv2['vo']
    ...
    for tc in range(tcmax):
        ...
        for t in range(trmax-trmin):
            ...
            for i in range(lonN*latN):
                if nomask[i]:
                    ...
                    #if i == ijtest:
                    #	print type(npy.interp(s_s[:,i], szm[:,i], zzm[:,i], right = 0., left = 0.)) ; # depth - consider spline
        ...
        #code.interact(banner='end of t loop', local=dict(locals(), **globals()))
		...
		print 'salt inim',x2Bin[inim,:,jtsti,itsti]
 		...
 		print 'maskp',maskp[:,ijtsti]
                ...
                for i in range(latN*lonN):
                   if p_top[i] == 0:
                      if nomask[i]:
                         if npy.max((npy.ma.reshape(depthBin,(tcdel,N_s+1,lonN*latN)) ) [:,bottom_ind[0,i],i], axis=0) > 300.:
                            counti = counti + 1
                            maskp[bottom_ind[0,i]-1,i] = True
