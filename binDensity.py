...
def densityBin(fileT,fileS,fileFx,targetGrid='none',fileV='none',outFile='out.nc',debug=True,timeint='all',mthout=False,gridfT='none',gridfS='none',gridfV='none'):
    ...
    - EG + JM 07 May 2018 - add meridional stream function (MSF) calculation
    - EG     Feb 2019   - corrected bug on zonal mean of integral fields
    - EG  18 Feb 2019   - add MSF as option (i.e. when fileV='none' or is not specified)
    ...
    if not os.path.exists(os.path.join(*outFile.split('/')[0:-1])):
        ...
        # TODO: Need to convert to shutil - tree create
    ...
    if so_h.shape[3] != thetao_h.shape[3] or so_h.shape[2] != thetao_h.shape[2] \
        ...
        return
    if fileV != 'none':
        if vo_h.shape[1] != thetao_h.shape[1] or vo_h.shape[0] != thetao_h.shape[0]:
            print '** Input variables have different dimensions, exiting..'
            return
    ...
    for tc in range(tcmax):
        ...
        if fileV != 'none':
            x3_bin = npy.ma.ones([tcdel, N_s+1, latN*lonN])*valmask
        tucz0     = timc.clock()
        if cpuan:
            cpu1 = 0.
            cpu2 = 0.
        ...
        for t in range(trmax-trmin):
            ...
            if debug and (t == 0 or t==6):
                ...
                if fileV != 'none':
                    print '  Mean meridional transport in z coordinates source grid (m2/s) : ', hvmtot
            ...
            if fileV != 'none':
                c3_s = npy.ma.ones((N_s+1, lonN*latN))*valmask
            tcpu1 = timc.clock()
            ...
            # Now testing with griddata (Yona Aug 30th 2019)

            #szm[szm>valmask/10] = npy.nan # Replacing valmask by nan for griddata interpolation
            #szm = maskVal(szm,valmask) # Create mask for szm for interpolation
            #zzm = maskVal(zzm,valmask)
        
            tcpu3 = timc.clock()
            for i in range(lonN*latN):
                if nomask[i]:
                    #z_s [0:N_s,i] = griddata(szm[:,i], zzm[:,i], s_s[:,i], fill_value=0.)
                    indi = npy.ma.flatnotmasked_edges(szm[:,i]) # Find 1st and last indices with unmasked data
                    if indi[0] != indi[1]:
                        z_s [0:N_s,i] = npy.interp(s_s[:,i], szm[indi[0]:indi[1]+1,i], zzm[indi[0]:indi[1]+1,i])   
                    #z_s [0:N_s,i] = npy.interp(s_s[:,i], szm[szm[:,i]<valmask/10,i], zzm[szm[:,i]<valmask/10,i], right = 0., left = 0.)
                    #z_s [0:N_s,i] = npy.interp(s_s[:,i], szm[:,i], zzm[:,i], right = 0., left = 0.)   
    ...
    #                         print(c1_s[0:N_s,i])
            ...
            if t==0 or t==6:
                import code
                code.interact(banner='end of interp', local=dict(locals(), **globals()))
#             z_s[npy.isnan(z_s)==True] = valmask # Replacing nan by valmask
#             szm[npy.isnan(szm)==True] = valmask
#             c1_s[npy.isnan(c1_s)==True] = valmask
#             c2_s[npy.isnan(c2_s)==True] = valmask
            ...
            if fileV != 'none':
                ...
                if debug and t == 0:
                    print ' bottom correction', bottom_ind[0,ijtest], bottom_ind[1,ijtest]
            ...
            if debug and (t==0 or t==6):
                import code
                code.interact(banner='end of t=0 or 6', local=dict(locals(), **globals()))        
        #
        # end of loop on t <===
        #
        # CPU analysis
        ...
        import code
        code.interact(banner='end of loop on t', local=dict(locals(), **globals()))

        # Free memory
        del(rhon, x1_content, x2_content,  vmask_3D, szm, zzm, c1m, c2m,  \
            z_s, c1_s, c2_s,  t_s, inds, c1_z, c2_z   ) ; gc.collect()
        if fileV != 'none':
            del (x3_content, c3m, c3_s, c3_z, x3intz, c3ders) ; gc.collect()
...
#         code.interact(banner='After wash mask', local=dict(locals(), **globals()))
        ...
        if fileV != 'none':
            x3Bin    = cdm.createVariable(x3_bin   , axes = rhoAxesList, id = 'hvm')
        #
        import code
        code.interact(banner='thickBin creation', local=dict(locals(), **globals()))
        ...
        if tcdel >= 12:
            ...
            if fileV != 'none':
                x3y  = cdm.createVariable(x3y, axes = rhoAxesList, id = 'isonx2y')
            ...
            for t in range(nyrtc):
                for ks in range(N_s+1):
                    ...
                    if fileV != 'none':
                        ...
                        x3Bini[t,ks,:,:].mask       = maski
                        x3Binia[t,ks,:,:]           = x3Bini[t,ks,:,:]*1.
                        x3Binia[t,ks,:,:].mask      = maskAtl
                        x3Binip[t,ks,:,:]           = x3Bini[t,ks,:,:]*1.
                        x3Binip[t,ks,:,:].mask      = maskPac
                        x3Binii[t,ks,:,:]           = x3Bini[t,ks,:,:]*1.
                        x3Binii[t,ks,:,:].mask      = maskInd
            ...
            if fileV != 'none':
                ...
                x3bini     = cdm.createVariable(x3Bini   ,  axes = [timeyr, rhoAxis, lati, loni], id = 'hvmg')
            ...
            if tc == 0:
                ...
                if fileV != 'none':
                    x3bini.long_name     = 'Volume flux'
                    x3bini.units         = 'm2/s'

            tozi = timc.clock()

            # Compute zonal mean (or integral for volume flux)
            # Global
            ...
            volBinz  = npy.ma.sum(thickBini *(1- (thickBini.mask).astype(int))*areaitsig, axis=3)
            volBinza = npy.ma.sum(thickBinia*(1- (thickBinia.mask).astype(int))*areaitsig, axis=3)
            volBinzp = npy.ma.sum(thickBinip*(1- (thickBinip.mask).astype(int))*areaitsig, axis=3)
            volBinzi = npy.ma.sum(thickBinii*(1- (thickBinii.mask).astype(int))*areaitsig, axis=3)
            ...
            for t in range(nyrtc):
                ...
                #idxvm = 1 - npy.ma.getmaskarray(thickBin[inim:finm,:,:,:]).astype(int)
                ...
                print 'idxvm in theoretical bowl',idxvm[:,54:,jtsti,itsti]
                print 'thick JAS in theoretical bowl',thickBin[6:9,54:,jtsti,itsti]
                print 'persist',persist[t,:,jtsti,itsti]
                print 'thick inim',thickBin[inim,:,jtsti,itsti]
                print 'salt inim',x2Bin[inim,:,jtsti,itsti]
                code.interact(banner='Persist calculation', local=dict(locals(), **globals()))
                ...
                thickrij       = thickBini.data[t,...]*(1-(thickBini.mask[t,...]).astype(int))
                temprij        = x1Bini.data[t,...]*(1-(thickBini.mask[t,...]).astype(int))
                salrij         = x2Bini.data[t,...]*(1-(thickBini.mask[t,...]).astype(int))
