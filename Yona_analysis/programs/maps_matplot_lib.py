...
def zonal_2D(plt, action, ax0, ax1, ticks, lat, density, varBasin, domrho, cmap, levels, clevsm=None, clevsm_bold=None):
    ...
    if action == 'ToE':
        bowl2 = varBasin['bowl_CO2']
        bowl1 = varBasin['bowl_piC']
        label1 = varBasin['labBowl'][0]
        label2 = varBasin['labBowl'][1]
    ...
    if action == 'ToE' or action == 'total_mme':
        ax0.plot(lat, bowl2, linestyle = '--', linewidth=2, color='black')
        ax0.plot(lat, bowl1, linewidth=2, color='black')
    ...
    # For selecting specific domains, make a fine grid
    ...
    if action == 'ToE' or action == 'total_mme':
        ...
        plt.legend(loc='upper right', title='Bowl', fontsize=12)
