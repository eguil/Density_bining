...
# import code
# code.interact(banner='arret1', local=dict(locals(), **globals()))
...
## REECRIRE BIEN CE BOUT AVEC JUSTE CE QU'ON GARDE
...
# ToE reference boxes
boxes1 = ax.boxplot(data1, vert=0, positions=ind, widths=width, whis=0,flierprops=red_crosses)
...
ax2.set_xlim([-1,6.01])
ax2.set_yticks(ind)
ax2.set_yticklabels(labels, horizontalalignment = 'left')
...
ax2.xaxis.set_tick_params(which='major',width=2)

plt.setp(ax2.get_yticklabels(), fontsize=12,fontweight='bold') #visible=True, 
...
plt.subplots_adjust(left=0.15,right=0.87,top=0.95,bottom=0.11)
...
ax2.text(-1.8,ind[11], 'Subpolar \n North Pacific', ha='center', va='center',fontweight='bold', fontsize=14)
...
#ax2.text(2.1,ind[11]-0.25, title1 + ' ('+str(nmodels1)+' models, '+str(nruns1)+' runs)', color='#c90016',
#         va='center', ha='left', fontweight='bold')
#ax2.text(2.1,ind[11]+0.25, title2 + ' ('+str(nmodels2)+' models, '+str(nruns2)+' runs)', color=color,
#         va='center', ha='left', fontweight='bold')
...
for i in range(len(newticknames2)):
    if i%2 == 0:
        ax3.hlines(y=-1.05, xmin=rangeticks[i,0], xmax=rangeticks[i,1],linewidth=1.5,color='k')
        ax3.text((rangeticks[i,0] + rangeticks[i,1])/2, -1.05, newticknames2[i], fontweight='bold', va='bottom', ha='center')
    elif i==1 or i==5:
        ax3.hlines(y=-1.205, xmin=rangeticks[i,0], xmax=rangeticks[i,1],linewidth=1.5,color='k')
        ax3.text((rangeticks[i,0] + rangeticks[i,1])/2, -1.25, newticknames2[i], fontweight='bold', va='top', ha='center')
    else:
        ax3.hlines(y=-1.35, xmin=rangeticks[i,0], xmax=rangeticks[i,1],linewidth=1.5,color='k')
        ax3.text((rangeticks[i,0] + rangeticks[i,1])/2, -1.4, newticknames2[i], fontweight='bold', va='top', ha='center')

plotName = 'GSATatE_boxplot_rcp85_histNat_2std_'+method_noise_histNat
