...
if name == 'mme':
    for ilat in range(len(lat)):
        if np.ma.is_masked(bowlpiC_a[ilat]) == False :
            inda = np.ma.nonzero(bowlpiC_a[ilat]>=density)
            toe_a[inda,ilat] = np.ma.masked
        if np.ma.is_masked(bowlpiC_p[ilat]) == False :
            indp = np.ma.nonzero(bowlpiC_p[ilat]>=density)
            toe_p[indp,ilat] = np.ma.masked
        if np.ma.is_masked(bowlpiC_i[ilat]) == False :
            indi = np.ma.nonzero(bowlpiC_i[ilat]>=density)
            toe_i[indi,ilat] = np.ma.masked

# -- Points where signal has not emerged (toe = 140) --> put value to 141 to put it in another color on the figure
toe_a[toe_a == 140] = np.ma.masked
toe_p[toe_p == 140] = np.ma.masked
toe_i[toe_i == 140] = np.ma.masked
